  ...
  const columns = [
    ...
    }, 
    {
      ...
      formatter: (value, row) => {
        return (
          <div>
            <h6 className="simple-font" style={{textAlign:"left"}}>{(value * row.price).toFixed(2)}</h6>
            <h6 className="simple-font" style={{textAlign:"left", color: "#9C9EAC" }}>
              {value}
            </h6>
          </div>
        );
        ...
        >
          {(props) => (
            <>
              ...
              >
                <div className="record-modal-title">持仓</div>
                <SearchBar
                  {...props.searchProps}
                  style={{
                    background: "#F5F6F8",
