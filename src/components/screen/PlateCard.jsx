...
export default function PlateCard({dataSource, heightProp,stockData,index}) {
  ...
  const columns = [
    {
      ...
      formatter: (value, row) => {
        return (
          <div>
            <h6 style={{textAlign:"left"}} className="stock-text">{value}</h6>
          </div>
        );
      },
    },
    {
      ...
      text: "股票名称",
      sort: true,
      style: { width: "16%", border: "none",textAlign:"left",padding: "12px 10px", },
      formatter: (value, row) => {
        return (
          <div>
            <h6 style={{textAlign:"left"}} className="stock-text">{value}</h6>
